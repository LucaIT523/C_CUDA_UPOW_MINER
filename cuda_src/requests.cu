...
json_object *curl_get(const char *url) {
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) {
            printf("curl_get() curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
//            exit(1);
        }
...
json_object *curl_post(const char *url, const char *data) {
    ...
    if (curl) {
        ...
        res = curl_easy_perform(curl);
        if (res != CURLE_OK) {
            printf("curl_post() curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
//            exit(1);
        }
...
//    strcat(url, "share");
    ...
    json_object_object_add(jobj, "block_no", json_object_new_int(id));
    ...
    if (!share.ok && json_object_object_get(response, "error") != NULL) {
        strncpy(share.error, json_object_get_string(json_object_object_get(response, "error")), 128);
    }
    if(response != NULL){
        strncpy(share.error, json_object_to_json_string_ext(response, JSON_C_TO_STRING_PRETTY), 128);
    }

    if(jobj) free(jobj);
    if(response) free(response);
    if(jarray) free(jarray);

    return share;
}

MiningInfo get_mining_info(const char *nodeUrl) {
    MiningInfo response = { 0, };
