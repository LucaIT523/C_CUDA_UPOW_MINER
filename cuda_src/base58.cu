...
bool b58tobin(void *bin, size_t *binszp, const char *b58, size_t b58sz)
{
    ...
    b58_almostmaxint_t *outi = (b58_almostmaxint_t*)malloc(sizeof(b58_almostmaxint_t) * outisz);//error
...
//    uint8_t buf[size];
    uint8_t *buf = (uint8_t*)malloc(sizeof(uint8_t) * size);//error
    memset(buf, 0, size);
...
//    uint8_t buf[1 + datasz + 0x20];
    uint8_t* buf = (uint8_t*)malloc(sizeof(uint8_t) * (1 + datasz + 0x20));//error
