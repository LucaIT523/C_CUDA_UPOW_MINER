...
__device__ void sha256_init_dev(SHA256_CTX *ctx);
__device__ void sha256_update_dev(SHA256_CTX *ctx, const BYTE data[], size_t len);
__device__ void sha256_final_dev(SHA256_CTX *ctx, BYTE hash[]);
...
__device__ void sha256_transform_dev(SHA256_CTX *ctx, const BYTE data[])
{
...
__device__ void sha256_update_dev(SHA256_CTX *ctx, const BYTE data[], size_t len)
{
    ...
    for (i = 0; i < len; ++i) {
        ...
        ctx->datalen++;
        if (ctx->datalen == 64) {
            sha256_transform_dev(ctx, ctx->data);
            ctx->bitlen += 512;
            ctx->datalen = 0;
        }
    }
