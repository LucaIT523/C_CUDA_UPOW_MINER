...
int main()
{
    ...
    GpuSettings     w_stGpuSettings;
    ManagerData     w_stManagerData;

    start(&w_stGpuSettings, &w_stManagerData);
...
cudaError_t addWithCuda(int *c, const int *a, const int *b, unsigned int size)
{
    ...
    char    w_Data[64] = { 0, };
    checkCudaErrors(cudaMemcpyToSymbol(share_chunk_c, w_Data, sizeof(char) * 64));
